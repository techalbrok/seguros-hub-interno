
import React from 'react';

export const PrivacyPolicyText = () => (
  <>
    <p><strong>Última actualización:</strong> 15 de junio de 2025</p>
    
    <h2>1. Responsable del Tratamiento</h2>
    <p>HubCore ("nosotros", "nuestro") es el responsable del tratamiento de sus datos personales.</p>
    
    <h2>2. ¿Qué datos recopilamos?</h2>
    <p>A través de nuestro formulario de suscripción, recopilamos los siguientes datos personales:</p>
    <ul>
      <li>Nombre</li>
      <li>Dirección de correo electrónico</li>
    </ul>
    
    <h2>3. ¿Con qué finalidad tratamos sus datos?</h2>
    <p>Tratamos los datos que nos proporciona con el fin de enviarle nuestro boletín de noticias, que incluye novedades, consejos y ofertas especiales sobre nuestros productos y servicios.</p>
    
    <h2>4. Legitimación para el tratamiento de datos</h2>
    <p>La base legal para el tratamiento de sus datos es su consentimiento explícito, que nos otorga al marcar la casilla de aceptación de la política de privacidad y hacer clic en el botón de suscripción.</p>
    
    <h2>5. ¿A qué destinatarios se comunicarán sus datos?</h2>
    <p>Sus datos (nombre y email) se comunicarán a nuestro proveedor de servicios de email marketing, MailRelay, con el único propósito de gestionar y enviar el boletín de noticias. MailRelay cumple con las normativas de protección de datos aplicables.</p>
    
    <h2>6. ¿Durante cuánto tiempo conservaremos sus datos?</h2>
    <p>Conservaremos sus datos mientras no solicite su supresión. Puede darse de baja en cualquier momento a través del enlace proporcionado en cada uno de nuestros correos electrónicos.</p>
    
    <h2>7. Sus derechos</h2>
    <p>Usted tiene derecho a acceder, rectificar, suprimir sus datos, así como otros derechos como se explica en la información adicional, contactándonos a través de un correo electrónico a legal@hubcore.com.</p>
  </>
);

export const CookiesPolicyText = () => (
  <>
    <p><strong>Última actualización:</strong> 15 de junio de 2025</p>
    
    <h2>1. ¿Qué son las cookies?</h2>
    <p>Una cookie es un pequeño fichero de texto que se almacena en su navegador cuando visita casi cualquier página web. Su utilidad es que la web sea capaz de recordar su visita cuando vuelva a navegar por esa página.</p>
    
    <h2>2. ¿Qué cookies utilizamos?</h2>
    <p>Este sitio web utiliza las siguientes cookies:</p>
    <ul>
      <li><strong>Cookies técnicas:</strong> Son esenciales para el funcionamiento del sitio web, como por ejemplo, gestionar la sesión.</li>
      <li><strong>Cookies de personalización:</strong> Nos permiten recordar sus preferencias, como el idioma o la región.</li>
    </ul>
    <p>Actualmente no utilizamos cookies de terceros para análisis o publicidad.</p>
    
    <h2>3. ¿Cómo desactivar las cookies?</h2>
    <p>Puede permitir, bloquear o eliminar las cookies instaladas en su equipo mediante la configuración de las opciones del navegador instalado en su ordenador. A continuación le ofrecemos enlaces en los que encontrará información sobre cómo puede activar sus preferencias en los principales navegadores:</p>
    <ul>
      <li>Google Chrome</li>
      <li>Mozilla Firefox</li>
      <li>Internet Explorer</li>
      <li>Safari</li>
    </ul>
  </>
);

export const LegalNoticeText = () => (
  <>
    <p><strong>Última actualización:</strong> 15 de junio de 2025</p>
    
    <h2>1. Datos Identificativos</h2>
    <p>En cumplimiento con el deber de información recogido en artículo 10 de la Ley 34/2002, de 11 de julio, de Servicios de la Sociedad de la Información y del Comercio Electrónico, a continuación se reflejan los siguientes datos:</p>
    <ul>
      <li><strong>Titular:</strong> HubCore S.L.</li>
      <li><strong>NIF:</strong> B12345678</li>
      <li><strong>Domicilio social:</strong> Calle Ficticia 123, 28080 Madrid, España</li>
      <li><strong>Email de contacto:</strong> legal@hubcore.com</li>
    </ul>
    
    <h2>2. Objeto</h2>
    <p>El presente Aviso Legal regula el uso del sitio web www.hubcore.com (en adelante, "el sitio web"), que HubCore pone a disposición de los usuarios de Internet.</p>
    <p>El sitio web ofrece una plataforma SaaS (Software as a Service) para corredurías de seguros, proporcionando información sobre sus características, beneficios y tecnología, así como un medio para solicitar una demostración o suscribirse a un boletín informativo.</p>
    
    <h2>3. Propiedad Intelectual e Industrial</h2>
    <p>Todos los derechos de propiedad intelectual del contenido de este sitio web y su diseño gráfico son propiedad exclusiva de HubCore, a quien corresponde el ejercicio exclusivo de los derechos de explotación de los mismos. Queda prohibida su reproducción, distribución, comunicación pública, total o parcial, sin la autorización expresa de HubCore.</p>
    
    <h2>4. Limitación de Responsabilidad</h2>
    <p>HubCore no se hace responsable de los daños y perjuicios de cualquier naturaleza que pudieran ocasionar, a título enunciativo: errores u omisiones en los contenidos, falta de disponibilidad del portal o la transmisión de virus o programas maliciosos en los contenidos, a pesar de haber adoptado todas las medidas tecnológicas necesarias para evitarlo.</p>
  </>
);

