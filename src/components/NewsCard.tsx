...
export const NewsCard: React.FC<NewsCardProps> = ({
  ...
  return <Card className="h-full hover:shadow-lg transition-shadow">
      ...
      <CardContent className="space-y-4">
        {news.featured_image && <div className="aspect-video rounded-lg overflow-hidden">
        ...
        <div className="space-y-2 text-xs text-gray-500">
          ...
          {news.companies && news.companies.length > 0 && <div className="flex items-center space-x-1">
              <Building2 className="w-3 h-3" />
              <span>{news.companies.length} compañía(s)</span>
            </div>}
          
          {news.categories && news.categories.length > 0 && <div className="flex items-center space-x-1">
              <Tag className="w-3 h-3" />
              <span>{news.categories.length} categoría(s)</span>
            </div>}
          
          {news.products && news.products.length > 0 && <div className="flex items-center space-x-1">
        ...
        <div className="flex gap-2 pt-2">
          <Button size="sm" variant="outline" onClick={() => onView(news)} className="flex-1">
          ...
          <Button size="sm" variant="outline" onClick={() => onDelete(news.id)} className="text-red-600 hover:text-red-700">
